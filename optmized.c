...
int main(int argc, char* argv[]){
	...
	fp = fopen("OptimizedParallelMultiplication.txt", "w+");
	printf("Test : OptimizedParallelMultiplication        \n");
    fprintf(fp, "Test : OptimizedParallelMultiplication        \n");
	fclose(fp);
...
void optimizedParallelMultiplyTest(int dimension, int iterations){
	 FILE* fp;
    fp = fopen("OptimizedParallelMultiplication.txt", "a+");
    ...
    for(int i=0; i<sampleSize; i++){
        TYPE** matrixResult = zeroSquareMatrix(dimension);
        opLatency[i] = optimizedParallelMultiply(matriA, matriB, matrixResult, dimension);
        free(matrixResult);
