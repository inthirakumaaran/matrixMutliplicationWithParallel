...
int main(int argc, char* argv[]){
	...
	fp = fopen("ParallelMultiplication.txt", "w+");
	printf("Test : ParallelMultiplication        \n");
    fprintf(fp, "Test : ParallelMultiplication        \n");
	fclose(fp);
...
void parallelMultiplyTest(int dimension, int iterations){
    FILE* fp;
    fp = fopen("ParallelMultiplication.txt", "a+");
    ...
    for(int i=0; i<sampleSize; i++){
        TYPE** matrixResult = zeroSquareMatrix(dimension);
        opLatency[i] = parallelMultiply(matriA, matriB, matrixResult, dimension);
        free(matrixResult);
